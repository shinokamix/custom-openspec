...
description: Propose a new change with all artifacts generated in one step. Use when the user wants to describe what they want to build and get specs and tasks ready for implementation.
...
- `specs/<capability-path>/spec.md` (behavior, shared contracts, and acceptance scenarios)
...
5. **Create every artifact in the required set**
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      ...
      - The required set is `applyRequires` plus every artifact reachable from those by following the `requires` edges in `status --json` - walk them transitively (spec-driven closes over specs and tasks). Leave artifacts outside that set alone
      ...
      - Skip one only when `status` already reports it `skipped`, or when its own `instruction` says it is conditional: run `openspec instructions <artifact-id> --change "<name>" --json` and skip only if its `instruction` field marks it optional. In spec-driven, `specs` qualifies only via the `skipped` status above, never by your own judgment. Tell the user, and do not reconsider it
